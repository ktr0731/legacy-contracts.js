...
DevPipe.prototype.transact = function (txPayload, callback) {
  ...
  this._burrow.txs().transact(from, to, txPayload.data, config.DEFAULT_GAS, config.DEFAULT_FEE, null, function (error, data) {
    if (error) {
      console.log(error)
      callback(error)
    } else {
...
DevPipe.prototype.transactAndHold = function (txPayload, callback) {
  var to = txPayload.to
  var from
